...
- Simple `task +` / `task -` lines double as an open-tasks list, with optional scheduling (a task can be given a future date and time and stays out of the active list, shown separately as "scheduled", until then). Active tasks can also be bulk-rescheduled to a later date from the tasks sheet.
